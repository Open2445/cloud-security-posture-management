...
const ExecutiveView: React.FC<ExecutiveViewProps> = ({ setActiveTab }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const findingsRes = await fetch(`${apiUrl}/api/findings`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (findingsRes.ok) {
  ...
  // Calculate Dasharray for SVG Circle
  const radius = 58;
      ...
      <div className="dashboard-grid">
        ...
        <div className="glass-panel col-4" style={{ textAlign: 'center', padding: '2rem' }}>
          ...
          <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginTop: '1.5rem', lineHeight: '1.4' }}>
            Postures evaluated across {stats.total_assets} resources. Standard deductions apply for open findings.
          </p>
        </div>
        ...
        <div className="glass-panel col-4" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          ...
          <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', lineHeight: '1.4' }}>
            Weighted risk score calculated by exposure level, IAM privilege, compliance scope, and lateral hops.
          </p>
        </div>
        ...
        <div className="col-4" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div className="glass-panel" style={{ display: 'flex', alignItems: 'center', gap: '1rem', flex: 1, padding: '1rem 1.5rem' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>Open Critical Risks</div>
            </div>
          </div>
          ...
          <div className="glass-panel" style={{ display: 'flex', alignItems: 'center', gap: '1rem', flex: 1, padding: '1rem 1.5rem' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>Discovered Cloud Assets</div>
            </div>
          </div>
            ...
            {Object.entries(stats.compliance_coverage).map(([framework, value]: any) => (
              <div key={framework}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', marginBottom: '0.4rem', fontWeight: 600 }}>
                  ...
                  <span style={{ color: 'var(--accent-color)' }}>{value}% compliant</span>
                </div>
        ...
        <div className="glass-panel col-6" style={{ padding: '1.75rem', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          <div>
            <h3 style={{ fontSize: '1.1rem', fontWeight: 700, fontFamily: 'var(--font-display)', marginBottom: '1.25rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
