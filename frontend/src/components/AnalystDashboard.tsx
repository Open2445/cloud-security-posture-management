...
const AnalystDashboard: React.FC = () => {
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/api/dashboard/stats?${params.toString()}`, {
  ...
  return (
    <div>
      ...
      <div className="glass-panel" style={{
        ...
        {loading && (
          <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Refreshing desk metrics...</span>
        )}
      </div>
                ...
                return (
                  <div key={sev}>
                    <div style={{ display: 'flex', justifyBetween: 'space-between', fontSize: '0.8rem', marginBottom: '0.25rem' }}>
                      ...
                      <span style={{ color: 'var(--text-secondary)', marginLeft: 'auto' }}>{count} ({Math.round(percentage)}%)</span>
                    </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
              <div>
                <div style={{ display: 'flex', justifyBetween: 'space-between', fontSize: '0.8rem', marginBottom: '0.25rem' }}>
                  ...
                  <span style={{ fontWeight: 700, color: 'var(--status-resolved)', marginLeft: 'auto' }}>{stats.remediation_progress.resolved}</span>
                </div>
                <div style={{ width: '100%', height: '6px', background: 'var(--bg-tertiary)', borderRadius: '3px', overflow: 'hidden' }}>
                  <div style={{ 
              ...
              <div>
                <div style={{ display: 'flex', justifyBetween: 'space-between', fontSize: '0.8rem', marginBottom: '0.25rem' }}>
                  <span style={{ color: 'var(--text-secondary)' }}>Snoozed Risks</span>
                  <span style={{ fontWeight: 700, color: 'var(--status-snoozed)', marginLeft: 'auto' }}>{stats.remediation_progress.snoozed}</span>
                </div>
                <div style={{ width: '100%', height: '6px', background: 'var(--bg-tertiary)', borderRadius: '3px', overflow: 'hidden' }}>
                  <div style={{ 
              ...
              <div>
                <div style={{ display: 'flex', justifyBetween: 'space-between', fontSize: '0.8rem', marginBottom: '0.25rem' }}>
                  ...
                  <span style={{ fontWeight: 700, color: 'var(--status-open)', marginLeft: 'auto' }}>{stats.remediation_progress.open}</span>
                </div>
                <div style={{ width: '100%', height: '6px', background: 'var(--bg-tertiary)', borderRadius: '3px', overflow: 'hidden' }}>
                  <div style={{ 
          ...
          <div className="glass-panel col-8" style={{ padding: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                ...
                <div key={e.id} style={{
                  ...
                  <div style={{ display: 'flex', gap: '0.75rem' }}>
                    <div style={{ 
                      ...
                      height: '8px', 
                      borderRadius: '50%', 
                      backgroundColor: e.severity === 'critical' ? 'var(--severity-critical)' : (e.severity === 'high' ? 'var(--severity-high)' : 'var(--text-muted)'),
                      marginTop: '0.35rem',
                      flexShrink: 0
                    }} />
                    <div>
              ...
              {stats.recent_events.length === 0 && (
                <div style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '2rem' }}>No events matches current criteria parameters.</div>
              )}
            </div>
          </div>
