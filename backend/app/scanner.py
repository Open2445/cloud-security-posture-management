...
def rebuild_attack_paths(db: Session) -> List[AttackPath]:
    # Clear existing attack paths
    db.query(AttackPath).delete()
    db.commit()

    # Find public EC2 instances, their security groups, and attached policies to trace Attack Path 1:
    # Internet -> Public EC2 -> IAM Role -> Admin Policy -> Sensitive S3
    ...
    # Path 1: Internet -> Public EC2 -> Role -> Policy -> S3
    ...
    # Path 2: Public Lambda -> Assume Role -> Secrets Manager -> RDS Database
...
def evaluate_findings(db: Session) -> int:
    ...
    # We will build attack paths first, then assign in_attack_path to findings
    ...
    for asset in assets:
        ...
        # --- IAM Permission Analyzer specific checks ---
        # 11. AdministratorAccess Role Attached
        ...
        # 13. Inactive Users with active access keys
...
def calculate_security_score(db: Session) -> float:
    ...
    # We apply deductions based on open findings linked to it:
    # Critical (-30), High (-15), Medium (-5), Low (-1). Capped at 0.
    assets = db.query(Asset).all()
    if not assets:
        return 100.0
    ...
    total_health = 0.0
    for asset in assets:
        # Get open findings for this asset
...
def trigger_cloud_asset_discovery(db: Session) -> ScanHistory:
    ...
    try:
        ...
        # Add security event for scanning completion
        event = SecurityEvent(
